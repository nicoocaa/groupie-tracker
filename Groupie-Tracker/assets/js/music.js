  document.addEventListener('DOMContentLoaded', function () {
        ...
        function toggleMusic(card) {
            ...
            var artistName = card.querySelector('p').textContent.trim();
            var deezerURL = artistURLs[artistName];
            let controlPanelObj = this.controlPanel,
            infoBarObj = this.infoBar,
            progressBarObj = this.progressBar;
    
            if (deezerURL) {
                if (currentAudioURL !== deezerURL) {
                    audio.src = deezerURL;
                    audio.play();
                    controlPanelObj.classList.add('active');
                    infoBarObj.classList.add('active');
                    currentAudioURL = deezerURL;
                    progressBarObj.classList.add('fill-animation');
                } else {
                    if (!audio.paused) {
                        audio.pause();
                        controlPanelObj.classList.remove('active'); 
                        infoBar.classList.remove('active');
                        progressBarObj.classList.remove('fill-animation');
